...
var createListSectionItems = function(items, poolInfo, roundInfo) {
	...
	_.map(items, function(item) {
		var itemData = {
			...
			match : {
				text : "Match: " + item.homeTeam + " VS " + item.awayTeam
			},
			wrapper : {
				height : Ti.UI.SIZE,
				top : 5,
		...
		if (item.venue_allocations && item.venue_allocations[0]) {
			itemData.matchDate = {
				text : "Date: " + item.venue_allocations[0].date_and_time
			};
			itemData.ground = {
				text : "Ground: " + item.venue_allocations[0].pitch.ground.name
			};
		}
		...
		if (item.scorecardIds && item.scorecardIds.length >= 1) {
			itemData.scorecardId = item.scorecardIds[0].description;
		}
...
$.fixturesList.addEventListener('itemclick', function(e) {
	var item = e.section.getItemAt(e.itemIndex);
	if (item.scorecardId) {
		var dialog = Ti.UI.createAlertDialog({
			title : 'Match Scorecard.',
			message : 'Navigating to CricHQ, would you like to view match scorecard?',
		...
		dialog.addEventListener('click', function(e) {
			if (e.index === e.source.cancel) {
				...
				Ti.Platform.openURL("https://www.crichq.com/matches/" + item.scorecardId + "/1st_innings");
			}
		});
		dialog.show();
	} else {
